...
- 在没有更高优先级指令要求额外确认时，对证据充分、范围明确、低风险、可回退且不改变既定目标的建议，说明判断依据和影响边界后在同一轮直接执行；用户的明确修改指令本身视为对该项最小实现的授权，不再把“是否确认”作为惯例步骤。
- 用户已经确认某一方向后，同一连续任务内为完成该方向所需的最小修正、测试契约同步、命名或样式一致性收口、文档同步和验证视为已授权；后续短反馈作为约束增量直接吸收，不重开整套方案确认。
- 只有存在会显著改变结果的多种方案，涉及架构、业务契约、数据模型、依赖、长期规则、破坏性或外部写入，执行范围需要扩大、证据不足，或更高优先级指令/skill 明确要求审批时才暂停；需要确认时合并成一次，只询问真正影响结果的决策。
...
- 不扫描无关目录：探索、定位、批量搜索前先判断任务实际涉及的目录/模块范围，`ls`/`find`/`rg` 等命令限定在该范围内，不对整个仓库根目录或明显无关的目录（如其他模块、第三方依赖、构建产物、历史归档）做探索性全量扫描；确实需要跨目录定位时，先用 `rg` 做定向关键字匹配缩小范围，而不是先整体罗列再人工筛选。
...
- 判断一个文件/模块该放全局目录还是局部模块目录，不能只看它当前物理位置；要用真实调用方范围做证据（搜索实际 import/引用方），调用方全部落在某个局部模块子树内时按真实归属收回该模块，不因为历史上放在全局目录就默认它该继续待在那。
- 把文件/模块移动或归并进另一个模块目录前，先检查这次移动会不会让"更通用/更全局的一方反向依赖更局部的一方"（例如全局组件被迁入某页面私有目录后，其它全局调用方要跨页面反向引用该页面内部路径）；会形成这种反向依赖时，先向用户点出风险和候选方案，不擅自执行。
- 模块内部按内容性质分区，不按"是否被同一处调用"分区：只有真正的展示/渲染单元（如前端组件）才归入模块内的 `components/` 一类子目录，纯逻辑、类型、model、常量等支撑文件放在模块根目录，不因为"也是给组件用的"就一起塞进子目录。
- 去除子文件/组件重复前缀（目录已表达上下文）时，范围默认限定在文件名和该文件的主要导出名，不自动级联到该文件内部引用的其它共享文件的所有导出标识符；要扩大重命名范围需先跟用户确认。重命名或移动导致与既有文件/模块同名冲突时，停下来向用户展示可选处理方案，不擅自选定谁改名、改成什么。
...
- URL 用名词复数表示资源集合（`/orders`），路径里不出现动词（不写 `/order/create`、`/getOrderList`）；操作语义由 HTTP 方法承担：GET 查、POST 建、PUT 全量替换、PATCH 局部更新、DELETE 删。
- 单个资源用 `/orders/{id}`；从属资源用嵌套路径表达归属（`/stores/{id}/pickup-times`）；过滤、排序、分页、搜索一律用 query 参数，不占用路径段。
- 用 HTTP 状态码表达结果类别（2xx 成功、4xx 调用方错误、5xx 服务端错误），不得一律返回 200 再靠响应体字段区分成败；响应体里的业务错误码只用于细分同一类别下的具体原因（如 422 下区分「已售罄」和「金额不一致」）。
- 确实无法映射为资源 CRUD 的领域动作或状态转换（取消、重新支付、确认到店等），允许 `POST /orders/{id}/{action}` 形式的动作端点，但必须在接口文档中写明为什么不能用资源语义表达。
- 认证、令牌刷新、文件上传、跨资源聚合查询等非资源端点可以豁免上述形式，但命名保持名词化且全项目风格统一。
- 同一资源在全项目只能有一种路径写法，禁止 `/order` 与 `/orders`、`/user` 与 `/users` 并存；新增或调整接口时同步更新接口文档、错误码表和全部调用方，并检查是否与既有资源命名冲突。
...
- `r0`：`animation-vocabulary`，`apple-design`，`bigfu-working-style`，`convert-markdown-to-docx`，`emil-design-eng`，`find-animation-opportunities`，`frontend-development`，`frontend-validation-optimization`，`Humanizer-zh`，`improve-animations`，`knowledge-graph`，`pick-ui-library`，`review-animations`，`write-case-study-reports`。
