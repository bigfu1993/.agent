...
- 同一个页面里上下相邻的多行栅格（指标行、图表行、内容卡行）必须**共用同一套列节奏**：先定一个基础列数（6 或 12），每行用 `grid-column: span N` 组合出想要的宽度，不要每行各写一套 `fr` 比例。三行分别写 `1.6fr 1fr 1fr 1fr`、`1fr 1fr`、`1fr 1.1fr` 时，纵向分隔缝在每一行落在不同的 x 上，整页看起来是歪的，而单看任何一行都正常——这类问题只有整页一起看才暴露。
- 同一行内的卡片默认**拉平到等高**，不要随手写 `align-items: start`；确有「短卡不该被拉高」的理由时要在注释里写明，否则一行卡片底边参差就是缺陷。
- 卡片内的尾部说明、脚注、图例用 `margin-top: auto` 沉到卡片底部（卡片自身设 `display: flex; flex-direction: column`），这样等高之后底部元素也在同一条线上；一行里只要有一张卡带脚注，其余卡要么补脚注要么明确留空，不能一半沉底一半吊在半空。
- 结构相同的指标卡（标题 / 数值 / 副文本）即使某几张没有副文本，也要**保留占位并给每一行 `min-height`**，让标题、数值、副文本各自落在同一条基线上；靠内容自然高度对齐会因为字号或行数不同而错开。
- 数值字号有主次时（主指标大、次指标小），用 `display: flex; align-items: baseline` + 固定行高槽位对齐基线，不要靠 `margin-top` 凑。

### 复合控件几何一致性

- 滑块、进度条、分段轨道、时间轴等由轨道容器、选中填充、拖拽点和状态描边共同组成的控件，先确定一套可解释的几何关系；同一控件内的圆角、端点和描边必须属于同一形状语言，不得无意混用圆形拖拽点、直角选中色块与圆角容器。
- 动态选中范围需要独立圆角端点时，使用绝对定位填充层或伪元素，并通过共享变量或 `inherit` 复用轨道圆角；不要用会在动态分界处产生直角的渐变色段代替选中层，除非设计规格明确要求硬分段。
- 拖拽点的形状必须映射轨道几何：紧凑圆角轨道优先使用完整收纳在轨道内的等高窄圆角长方形；只有设计语义明确需要独立节点或圆形锚点时才使用圆点，不能因为原生 range 默认样式是圆形就直接沿用。
- hover、focus、active、selected 和 disabled 状态不得通过增减边框宽度改变控件尺寸；使用预留透明边框与 `border-box`、内描边或不参与文档流的 outline/shadow 保持盒模型稳定。
- 实现时至少覆盖空值、最短有效值、普通值、贴近两端边界、聚焦和禁用状态；动态值出现或消失时，轨道高度、所在行高和周边内容位置必须保持不变。
