...
- 前端开发、重构、UI/交互和页面验证必须先使用全局 skill `~/.claude/skills/frontend-development/SKILL.md`。
- 全局规则只保留入口规则；具体组件组织、状态归属、表单布局、响应式、本地运行进程管理、运行态验证和交付检查以 `frontend-development` skill 为准。
- 若项目 `AGENTS.md`/`CLAUDE.md`、设计系统或用户要求有更具体前端规则，先合并到当前任务约束，再按 `frontend-development` skill 执行。
